  ...
  const checkPermission = (canAccess?: Viewer[], permissions?: string[]) => {
    if (canAccess != undefined && canAccess != null && canAccess.length > 0 && user?.level != undefined) {
      ...
      if (permissions !== undefined && permissions.length > 0) {
        if (
          user?.role?.permissions?.map((el) => el.key).some((el) => permissions.includes(el)) ||
          user?.role?.permissions?.map((el) => el.key).includes("manage-all") ||
          user?.level == Viewer.ADMINISTRATOR
        ) {
          return true;
        } else {
          return false;
