...
export default function Page() {
  ...
  const getRows = () => {
    setIsLoading(true)
    axios.post(ApiEndpoint.USER_INDEX)
    .then((res) => {
  ...
  const deleteRow = () => {
    setIsLoading(true)
    axios.delete(ApiEndpoint.USER + "/" + deleteId)
    .then (() => {
      getRows()
    })
    .catch(err => {
  ...
  const columns: GridColDef<(typeof rows)[number]>[] = [
    ...
    {
      field: "sites",
      headerName: "Jumlah Cabang",
      width: 100,
      editable: false,
      type: "number",
      valueGetter: (_, row) => row.sites.length
    },
    {
      field: "is_active",
      headerName: "Aktif",
    ...
    },
    {
      ...
      getActions: (params) => [
        ...
        <GridActionsCellItem
          ...
          label="Ubah"
          onClick={() => navigate("/ubah-user/"+params.row.id)}
          showInMenu
        />,
      ],
    },
  ...
  return (
    <Grid container spacing={5}>
      ...
      <DetailUser openDialog={openDetail} setOpenDialog={setOpenDetail} data={activeData} />
      <Grid size={12}>
        <DataGrid
          loading={isLoading}
          rows={rows}
