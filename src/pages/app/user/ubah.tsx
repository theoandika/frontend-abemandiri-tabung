...
export default function UbahUser() {
  ...
  const getInitData = () => {
    setIsLoading(true)
    axios.get(ApiEndpoint.USER + "/" + id)
    .then((res) => {
      setName(res?.data?.data?.name)
      setEmail(res?.data?.data?.email)
      setRole(res?.data?.data?.role?.id)
      setSites(res?.data?.data?.sites?.map((el: any) => el?.id))
  ...
  const save = () => {
    ...
    axios.post(ApiEndpoint.USER + "/" + id, data)
    .then(() => {
      navigate('/user')
    })
    .catch((err) => {
              ...
              <Grid container spacing={2}>
                ...
                <Grid size={{ xs: 12, md: 6 }}>
                  <FormControl className="outlined" variant="standard" size="small" fullWidth>
                    ...
                    <Input type="email" placeholder="" value={email} onChange={e => setEmail(e.target.value)} disabled={true} />
                    {errors != undefined && errors['email'] && <FormLabel component="label" className="text-error! mt-0.25 text-sm!">{errors['email'][0]}</FormLabel>}
                  </FormControl>
                </Grid>
                <Grid size={{ xs: 12, md: 6 }}>
                  <FormControl className="outlined" variant="standard" size="small" fullWidth>
                    <FormLabel component="label">Password <Typography variant="caption">(Masukkan jika ingin mengganti password)</Typography></FormLabel>
                    <Input
