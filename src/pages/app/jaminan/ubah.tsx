...
export default function UbahJaminan() {
  ...
  const initData = () => {
    setIsLoading(true)
    axios.get(ApiEndpoint.COLLATERAL + "/" + id)
    .then((res) => {
      let data = res.data?.data
      setSite(data?.site?.id)
      setDate(dayjs(data?.date))
      setMember({ label: `${data?.member.code} - ${data?.member.name}`, value: data?.member?.id })
      setType(data?.type)
      setDocumentNumber(data?.document_number)
      setCurrentDocument(data?.document)
      setPic(data?.pic)
      setMemberName(data?.member_name)
      setMemberAddress(data?.member_address)
      setSignatoryStatus(data?.signatory_status)
      setCompanyName(data?.company_name)
      setContactPerson(data?.contact_person)
      setPaymentMethod(data?.payment_method)
      setPaymentDate(data?.payment_date ? dayjs(data?.payment_date) : null)
      setReturnPaymentMethod(data?.return_payment_method)
      setReturnPaymentDate(data?.return_payment_date ? dayjs(data?.return_payment_date) : null)
      setCollateralAudit(data?.collateral_audit)
      setReturnAudit(data?.return_audit)
      setCollateralItems(data?.collateral_items?.map((el: any) => ({
        id: crypto.randomUUID(),
        tube_content_type: {
          label: `${el.tube_content_type.code} - ${el.tube_content_type.name}`,
          value: el.tube_content_type.id
        },
        klep_condition: el.klep_condition,
        tube_cap: el.tube_cap,
        tube_quantity: el.tube_quantity,
        nominal: el.nominal
      })))
    })
    .catch(() => {
  ...
  const save = () => {
    ...
    axios.post(ApiEndpoint.COLLATERAL + "/" + id, data)
    .then(() => {
      navigate('/jaminan')
    })
    .catch((err) => {
  ...
  useEffect(() => {
    if (!checkPermission([], ['update-collateral'])) {
      navigate('/404')
    } else {
      getSiteOptions()
      getMemberOptions()
                          ...
                          {document.length > 0 ? (
                            ...
                            <Box className="flex flex-col items-center justify-center w-full">
                              <Typography variant="body1" className="pointer-events-none w-full self-center text-center">
                                Upload untuk mengubah dokumen (.pdf)
                              </Typography>
                              {currentDocument && (
                                <Button
                                  ...
                                  color="info"
                                  variant="text"
                                  onClick={() => window.open(currentDocument ?? "", '_blank')?.focus()}
                                >Lihat</Button>
                              )}
                            </Box>
                          )}
