...
export default function UbahRole() {
  ...
  const getInitData = () => {
    setIsLoading(true)
    axios.get(ApiEndpoint.ROLE + "/" + id)
    .then(res => {
      setName(res?.data?.data?.name)
      setPermissions(res?.data?.data?.permissions?.map((el: { key: string, name: string }) => el?.key))
    })
  ...
  const save = () => {
    ...
    axios.post(ApiEndpoint.ROLE + "/" + id, data)
    .then(() => {
      navigate('/role')
    })
    .catch((err) => {
